...
A list of Cincinnati Public Schools and their addresses is taken from the [district website](https://www.cps-k12.org/findaschool).
...
The census data itself can be downloaded directly to R Studio using the tidycensus package. These include 62 specific variables, some of which will be combined into broader categories.
...
First, the data is downloaded for Cincinnati neighborhoods. Because some of these neighborhoods cross census tract lines, the data are selected at the more granular block group level.
...
If the address is placed in one and only one municipality, and that municipality is not Cincinnati, then the geocoding process for that address is complete.
...
Next, the locations of pharmacies in the CCHMC electronic health record are downloaded and extracted. These will be added to the school addresses for geocoding.
...
The addresses are geocoded using the tidygeocoder package to link to a Census Bureau API.
...
The latitude and longitude coordinates determined by the geocoders are then added to those for the school buildings, then matched with the county subdivision boundaries to determine which pharmacies are in Cincinnati and which are in the eight-county area.
...
For municipalities that cross county lines (Loveland, Milford, and Fairfield), the segments in different counties must be combined using the sf package.
...
Finally, the county map is created using the leaflet package.
